...
def create_product_routes(
    ...
    return controllers.create_product(body,db,user)
...
def update_product_routes(
    ...
    db:Session= Depends(get_db),
    user:UserModel= Depends(auth)):
    return controllers.update_product(product_id,body,db,user)
...
def delete_product_routes(
    product_id: int,
    db:Session= Depends(get_db),
    user:UserModel= Depends(auth)):
    return controllers.delete_product(product_id,db,user)
